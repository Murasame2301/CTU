...
function Is-NoteLine {
    ...
    return $normalized -match '^(ghi chu|luu y|mac du|->|\(giai thich:)'
}
...
function Remove-QuestionPrefix {
    ...
    $match = [regex]::Match($FullText, '^(C\S*u(\s*\d+)?|\d{1,2})\s*[:\.\-\)]\s*', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
