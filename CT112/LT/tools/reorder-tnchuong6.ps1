...
function Is-OptionLine {
    ...
    return $Text.Trim() -match '^[A-Ga-g]\s*[:\.\)]'
}
...
function Is-NoteLine {
    ...
    return (
        $normalized -match '^(ghi chu|luu y|mac du|->|\(giai thich:|answer:|router dua vao dau|hoi giao thuc nao|hoi dia chi nay thuoc lop nao|^-?\s*chon dap an chinh xac nhat)'
    )
}
...
function Is-ImplicitQuestionStart {
    ...
    $normalized = (Remove-Diacritics $trimmed).ToLowerInvariant()
    if ($normalized -match 'chon nhieu dap an') {
        return $true
    }
...
$docxPath = 'TNChuong6.docx'
$backupPath = 'TNChuong6.backup-before-reorder.docx'
