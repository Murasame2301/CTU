...
Object* readData(int *W,int *n){
    ...
    while(!feof(f)){
        fscanf(f,"%d%d%[^\n]",&pO[i].wei,&pO[i].val,&pO[i].name);
        pO[i].unit_price=(float)pO[i].val/pO[i].wei;
        pO[i].PA=0;
        i++;
...
void inDS(Object t[],int n, int w, int w_conlai){
    ...
    printf("|-----|------|------|------|--------------------|------|\n");
    printf("|-STT-|--TL--|--GT--|--DG--|-----Ten do vat-----|--PA--|\n");
    for(i=0;i<n;i++){
        printf("|%-5d|%-6d|%-6d|%-6.2f|%-20s|%6d|\n",i+1,t[i].wei,t[i].val,t[i].unit_price,t[i].name,t[i].PA);
    }
    printf("|-----|------|------|------|--------------------|------|\n");
